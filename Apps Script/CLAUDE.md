...
- `doGet()` sirve `Index.html`. El frontend hace **dos llamadas paralelas** por `google.script.run`: `getPuntosJson()` y `getBricksJson()`.
- **Ambos endpoints devuelven strings JSON (`JSON.stringify`), nunca objetos**: la serialización de objetos grandes/anidados de `google.script.run` es lenta y falla en silencio. El cliente hace `JSON.parse`. Mantener este patrón.
- Las columnas se localizan por fragmento de encabezado en minúsculas (`buscar_`), no por índice fijo — la hoja es un export de Salesforce y los nombres pueden variar levemente.
- `parseCoord_()` tolera números nativos, coma decimal ("4,7447"), separadores de miles y espacios; descarta (0,0) y fuera de rango. `getPuntosJson()` sigue devolviendo `stats` (filas / conCoordenada / sinCoordenada) aunque ya no hay un KPI que lo muestre en el front (se quitó por pedido explícito) — queda disponible en el payload para un futuro diagnóstico sin tener que abrir la hoja.
- Colores de bricks por ciudad (paleta categórica validada, orden fijo — no reordenar ni agregar tonos inventados): BOGOTA `#2a78d6`, MEDELLIN `#008300`, CALI `#e87ba4`, CARTAGENA `#eda100`. Ciudad nueva → siguiente slot de la paleta del skill dataviz.
...
- Filtros cruzados (`DIMENSIONES` en el script del cliente): agregar uno nuevo es una entrada en ese arreglo (`key`/`etiqueta`/`textoTodos`) — el `<div id="filtros">` se genera entero desde JS (`construirFiltros()`), no hay `<select>` en el HTML. `filtros[key]` es un **array** (acumulable): `[]` = sin filtro, si no solo pasan los que coincidan con alguno de los valores elegidos (OR dentro de la dimensión, AND entre dimensiones). El multi-select es un panel en línea (no `position:absolute`) para que el `overflow-y:auto` de `.side` no lo recorte — si se necesita un dropdown flotante en el futuro, hay que resolver el clipping antes. El buscador dentro de cada filtro solo aparece con ≥ `MIN_OPCIONES_BUSCADOR` opciones (en una lista de 3 estorba).
- **Tamaño de los puntos y zoom**: el radio es ∝ **raíz** de `Units YTD` (el área, no el radio, representa la magnitud) y además se multiplica por `escalaZoom()`. Sin ese factor, a nivel país los ~4.700 PDV se solapan hasta volverse una mancha verde sólida que tapa el mapa — verificado en navegador. Los markers guardan su radio sin escalar en `_radioBase` y `zoomend` los reescala (barato: `setRadius`, sin recrear la capa). Se pintan de mayor a menor venta para que los círculos chicos queden encima y ninguno desaparezca, y llevan aro blanco de 1px que los separa en zonas densas.
- Clic en un brick del mapa dispara `seleccionarBrick()`, que hace punto-en-polígono client-side (`puntoEnGeometria`, ray casting — misma idea que `Módulo Geocodificación.js` pero solo para acotar el panel, no para asignar el brick oficial de un PDV). Acota **las dos pestañas** del panel (resumen y lista) y salta a la de PDV, que es el detalle que se busca al entrar a un brick. Es independiente del filtro de texto "Brick" (`brickCrm`) — combinar ambos es útil para ver desalineaciones entre el brick geométrico real y lo que quedó tageado en Salesforce.
- `brickCrm` se normaliza en el backend (`cargarMapaBricks_()` en Code.js): si el CRM trae el código corto (`BOG-005`) en vez del nombre, se traduce contra `brick_id → nombre_brick` de la hoja `Bricks`. Si no hay match, se deja el texto libre tal cual (el gap real de homologación sigue sin resolver, esto solo tapa el caso más visible).
- La lista de PDV corta en `TOPE_LISTA_PDV` (300) filas y **lo dice en el encabezado** ("Top 300 de N"): el scroll es interno, pero pintar miles de nodos igual cuesta. Nunca truncar en silencio.
